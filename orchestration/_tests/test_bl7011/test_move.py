...
def test_process_new_7011_file_task(mocker: MockFixture) -> None:
    """
    ...
    # Patch get_transfer_controller where it is used in process_new_7011_file_task.
    mocker.patch(
    ...
    result = process_new_7011_file_task(file_path=test_file_path, config=mock_config)
    ...
    result = process_new_7011_file_task(file_path=test_file_path, config=None)
    assert mock_transfer_controller.copy.call_count == 1, "Transfer controller copy method should be called exactly once"
    assert result is None, "The flow should return None"
    assert mock_prune.call_count == 1, "Prune function should be called exactly once"
...
def test_dispatcher_7011_flow(mocker: MockFixture) -> None:
    ...
    This test verifies that:
      - The process_new_7011_file_task function is called with the correct parameters
    ...
    # Patch the process_new_7011_file_task function to monitor its calls.
    mock_process_new_7011_file_task = mocker.patch(
        "orchestration.flows.bl7011.dispatcher.process_new_7011_file_task",
        return_value=None
    )
    ...
    # Verify that process_new_7011_file_task is called even when config is None
    mock_process_new_7011_file_task.reset_mock()
    dispatcher(
        file_path=test_file_path,
        is_export_control=False,
        config=None
    )
    mock_process_new_7011_file_task.assert_called_once()
    ...
    mock_process_new_7011_file_task.assert_not_called()
