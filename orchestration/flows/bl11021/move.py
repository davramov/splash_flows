...
from orchestration.flows.bl11021.config import Config11021
...
@flow(name="new_11021_file_flow", flow_run_name="process_new-{file_path}")
def process_new_11021_file(
    file_path: str,
    config: Config11021
) -> None:
    """
    ...
    logger.info(f"Processing new 11021 file: {file_path}")
    ...
    transfer_controller.copy(
        file_path=file_path,
        source=config.data11021_raw,
        destination=config.nersc11021_alsdev_raw
    )
    ...
    prune(
        file_path=file_path,
        source_endpoint=config.data11021_raw,
        check_endpoint=config.nersc11021_alsdev_raw,
...
@flow(name="move_11021_flight_check", flow_run_name="move_11021_flight_check-{file_path}")
def move_11021_flight_check(
    file_path: str = "test_directory/test.txt",
):
    """Please keep your arms and legs inside the vehicle at all times."""
    logger.info("11021 flight check: testing transfer from data11021 to NERSC CFS")
    ...
    if success is True:
        logger.info("11021 flight check: transfer successful")
    else:
        logger.error("11021 flight check: transfer failed")
...
if __name__ == "__main__":
    ...
    process_new_11021_file(file_path, config)
