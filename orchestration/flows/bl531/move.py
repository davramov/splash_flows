...
def process_new_531_file(
    ...
    1. Copy the file from the data531 to NERSC CFS. Ingest file path in SciCat.
    2. Schedule pruning from data531. 6 months from now.
    ...
    logger.info(f"Processing new 531 file: {file_path}")
    ...
    transfer_controller.copy(
        file_path=file_path,
        source=config.bl531_compute_dtn,
        destination=config.bl531_nersc_alsdev
    )
    ...
    prune(
        file_path=file_path,
        source_endpoint=config.bl531_compute_dtn,
        check_endpoint=config.bl531_nersc_alsdev,
        days_from_now=180.0
    )
...
@flow(name="move_531_flight_check", flow_run_name="move_531_flight_check-{file_path}")
def move_531_flight_check(
    file_path: str = "test_directory/test.txt",
):
    """Please keep your arms and legs inside the vehicle at all times."""
    logger.info("531 flight check: testing transfer from data531 to NERSC CFS")
