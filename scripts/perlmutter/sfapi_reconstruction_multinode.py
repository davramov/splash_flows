...
def recon(
    ...
    *args, **kwargs
    ...
    Path(fulloutputPath).mkdir(parents=True, exist_ok=True)
    set_directory_permissions(fulloutputPath)  # Set permissions for the output directory
...
#					tomo = tomo[:, :, 10:-10]
                    ...
                    if projIgnoreList is not None:
                        for badproj in projIgnoreList:
                            tomo[badproj] = 0
                    rec = tomopy.recon(tomo, anglelist, center=cor+npad, algorithm=recon_algorithm, filter_name='butterworth', filter_par=[butterworth_cutoff, butterworth_order], ncore=64)
                    ...
                    if dorecon:
                        if sinoused[2] == 1:
                            ...
                            for sinotowrite in rec:    #fixes issue where dxchange only writes for step sizes of 1
                                ...
                                set_file_permissions(filenametowrite + '_' + '{0:0={1}d}_norm'.format(num, 5))  # Set permissions for individual files
                                num += sinoused[2]
                    else:
                        if verbose_printing:
...
def set_directory_permissions(path):
    os.chmod(path, stat.S_IRWXU | stat.S_IRWXG | stat.S_IROTH | stat.S_IXOTH | stat.S_ISGID)
...
if __name__ == '__main__':
    if len(sys.argv) < 3:
        print("Usage: python reconstruction.py <file_name> <folder_path> [sino_start] [sino_end]")
        sys.exit(1)

    file_name = sys.argv[1]
    folder_path = sys.argv[2] + '/'
    
    # Optional slice range for multi-node parallelism
    sino_start = int(sys.argv[3]) if len(sys.argv) > 3 else None
    sino_end = int(sys.argv[4]) if len(sys.argv) > 4 else None
    
    sinoused = None
    if sino_start is not None and sino_end is not None:
        sinoused = (sino_start, sino_end, 1)

    recon_dictionary, _ = recon_setup(filename=file_name,
                                      inputPath=folder_path,
                                      outputPath="../scratch/"+folder_path,
                                      sinoused=sinoused)
